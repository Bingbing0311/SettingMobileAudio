...
@property(nonatomic,strong)ZBBMobileSound *myPlaySounde;
@end
...
-(void)isMuted:(BOOL)muted
{
    ...
    if (muted) {
        //开启静音模式
        self.myPlaySounde = [[ZBBMobileSound alloc] initSystemShake];
    }else{
        //关闭静音模式
        NSString *name = [NSString stringWithFormat:@"%@",self.dataSource2[_selecetIndexpath.row]];
        self.myPlaySounde = [[ZBBMobileSound alloc] initSystemSoundWithName:name SoundType:@"caf"];
    }
